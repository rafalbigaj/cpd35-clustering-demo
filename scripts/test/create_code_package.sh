...
cpdctl asset file upload --space-id $qa_space_id --path code_package/clustering-credit-risk.zip --file code_package.zip

# Create the new code package
code_package_id=$(cpdctl code-package create --space-id $qa_space_id --file-reference code_package/clustering-credit-risk.zip --name "$code_package_name" --output json | jq -r '.asset_id')
